...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    authorizationHeader: undefined, // If non-empty, required value of the authorization header
  ...
  var hasauthorizationHeader = function(headers) {
    var header = headers['Authorization'] || headers['authorization']
    if (header) {
      header = header.trim()
    }
    return !corsAnywhere.authorizationHeader || header === corsAnywhere.authorizationHeader;
  }
  ...
  return function(req, res) {
    ...
    if (!hasauthorizationHeader(req.headers)) {
      res.writeHead(401, 'Unauthorized', cors_headers);
      res.end("Incorrect authorization token provided");
      return;
    }
